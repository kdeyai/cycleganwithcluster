...
class FRUNet(nn.Module):
  def __init__(self):
    ...
    self.mod11 = ModifiedResidualBlock(3, 32)
    ...
    self.conv = nn.Conv2d(32, 3, kernel_size=1, stride=1,bias=True)
    ...
    self.final = ModifiedResidualBlock(256, 256)

  def forward(self,x):

    if x.size()[1] == 1:
        x = x.expand((8, 3, 256, 256))
...
class NLayerDiscriminator(nn.Module):
    ...
    def forward(self, input):
        ...
        if input.size()[1] == 1:
            input = input.expand((8, 3, 256, 256))
